// components/panels/BoredCard.tsx
...
interface BoredCardProps {
  results: unknown[];
  apiName: string;
}

export const BoredCard: React.FC<BoredCardProps> = ({ results, apiName }) => {
      ...
      <div className="space-y-3 overflow-y-auto max-h-96">
          ...
          <div className="bg-gray-100 dark:bg-slate-700/30 rounded-lg p-6 border border-gray-300 dark:border-slate-600/30 transition-colors duration-200">
            <div className="flex items-center justify-center space-x-3">
              ...
              <div className="text-gray-700 dark:text-slate-300">
                <p className="text-sm">Finding activities...</p>
                {lastCommand && (
          ...
          <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <h4 className="text-red-300 font-medium mb-2">Activity Request Failed</h4>
        ...
        {!isLoading && !error && results.length === 0 && (
          <div className="text-center text-gray-600 dark:text-slate-400 py-8">
            ...
            <p className="text-xs mt-1">Try: "get activity"</p>
          </div>
        )}
        ...
        {!isLoading && results.length > 0 && (
          <>
            {results.map((apiResult: any, index) => {
              // Bored API returns a single activity object in apiResult.data
              const activity = apiResult.data;
              return (
                <div key={apiResult.id || index} className="bg-white dark:bg-slate-700/30 rounded-lg p-4 border border-gray-200 dark:border-slate-600/30 hover:bg-gray-50 dark:hover:bg-slate-700/40 transition-colors">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className="text-xs text-gray-500 dark:text-slate-500">
                      {new Date(apiResult.timestamp || activity.timestamp || Date.now()).toLocaleTimeString()}
                    </span>
                  </div>
                  
                  <p className="text-gray-700 dark:text-slate-300 leading-relaxed mb-3">{activity.activity}</p>
                  ...
                  <div className="flex justify-between items-center text-sm text-gray-600 dark:text-slate-400">
                    ...
                    <span>💰 {getPriceText(activity.price)}</span>
                  </div>
                </div>
              );
            })}
      ...
      {results.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-600/30">
          <div className="flex justify-between items-center text-xs text-gray-600 dark:text-slate-400">
            <span>{results.length} activit{results.length === 1 ? 'y' : 'ies'}</span>
            {filter && (
