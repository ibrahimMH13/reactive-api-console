// components/panels/CustomBackendCard.tsx
...
const CustomBackendCard: React.FC<CustomBackendProps> = ({ results, apiName }) => {
          ...
          <div className="bg-gray-100 dark:bg-slate-700/30 rounded-lg p-6 border border-gray-300 dark:border-slate-600/30 transition-colors duration-200">
            <div className="flex items-center justify-center space-x-3">
              ...
              <div className="text-gray-700 dark:text-slate-300">
                <p className="text-sm">Fetching from custom backend...</p>
                {lastCommand && (
          ...
          <div className="bg-red-900/20 border border-red-500/30 rounded-lg p-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <h4 className="text-red-300 font-medium mb-2">Backend Request Failed</h4>
        ...
        {!isLoading && results.length > 0 && (
          <>
            {results.map((apiResult: any, index) => (
              <div
                key={apiResult.id || index}
                className="bg-white dark:bg-slate-700/30 rounded-lg p-4 border border-gray-200 dark:border-slate-600/30 hover:bg-gray-50 dark:hover:bg-slate-700/40 transition-colors"
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-xs text-gray-500 dark:text-slate-500">
                    {new Date(apiResult.timestamp || Date.now()).toLocaleTimeString()}
                  </span>
                </div>
      ...
      {results.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-600/30">
          <div className="flex justify-between items-center text-xs text-gray-600 dark:text-slate-400">
            <span>{results.length} custom response{results.length !== 1 ? 's' : ''}</span>
            {filter && (
...
export default CustomBackendCard;